...
if __name__ == "__main__":
	...
	my_ip = get_ip()
	try:
		print(f"Trying to registry: {SERVER2_NAME} -> {my_ip}:{SERVER2_PORT}")
		server_name_conn.root.exposed_register(SERVER2_NAME, my_ip, SERVER2_PORT)
	except:
		print("Failed to register server in the name server")
		exit()
	print("Successfully registry in the name server")
	server = ThreadedServer(Calculator(), hostname= my_ip, port = SERVER2_PORT)
	server.start()